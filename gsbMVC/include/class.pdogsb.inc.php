...
*/	
	...
	public function cloturerFiche(){
		if (date('d') > 10){
			...
			$req="update fichefrais set idetat='CL' where mois='" . $annee . $mois . "' and idetat='CR'";
			PdoGsb::$monPdo->exec($req);
		}	
	}

	public function validerFiche($idvisiteur, $mois){
		$req="update fichefrais set idetat='VA' where idvisiteur='$idvisiteur', mois=$mois and idetat='CL'";
			PdoGsb::$monPdo->exec($req);	
	}
...
*/
	...
	public function changeLibelle($idvisiteur,$mois, $idLigne){
		$req = "update lignefraishorsforfait set libelle=CONCAT('REFUSE ',libelle) where idvisiteur='".$idvisiteur."'";
		$req .= " and mois='$mois' and id=$idLigne";
		PdoGsb::$monPdo->exec($req);
	}

/**	public function modifierFrais($idvisiteur,$mois){
		$req = "update lignefraisforfait set idvisiteur 
		where lignefraisforfait.idvisiteur ='$idvisiteur' and lignefraisforfait.mois='$mois' 
		order by lignefraisforfait.idfraisforfait";	
		PdoGsb::$monPdo->exec($req);
	}

	public function modifierFraisHorsForfait($idvisiteur,$mois){
		$req = ""
		PdoGsb::$monPdo->exec($req);
	}
/*
 ...
 */
	...
	public function misePaiement($idvisiteur,$mois,$etat){
		$req = "update fichefrais set idetat = 'RB', dateModif = now() 
		where fichefrais.idvisiteur ='$idvisiteur' and fichefrais.mois = '$mois' and and idetat='VA'";
		PdoGsb::$monPdo->exec($req);	
	}
